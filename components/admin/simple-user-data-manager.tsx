  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        link.href = url
        link.download = result.filename || 'users-export.xlsx'
        document.body.appendChild(link)
        link.click()
        document.body.removeChild(link)
        URL.revokeObjectURL(url)

        toast.success(`✅ Exported ${result.count} users successfully`)
      }
    } catch (error) {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
          <p className="text-xs text-blue-800">
            <strong>📋 Format:</strong> Excel file with columns: email, full_name, role, department, is_active
          </p>
          <p className="text-xs text-blue-700 mt-1">
            <strong>🔍 Validation:</strong> Prevents duplicate emails and validates required fields
          </p>
        </div>
      </CardContent>
    </Card>
